...
def evaluate_v2(
    predictions,
    gold_fpath='/home/jovyan/sharedstorage/s12b3v/dp/data/political_debates/test/test_combined.tsv',
    thresholds=None
):
    """
    Evaluates the predicted line rankings w.r.t. a gold file.
    ...
    If not specified - 1, 3, 5, 10, 20, 50, len(ranked_lines).
    """
    gold_df = pd.read_csv(gold_fpath, sep='\t', index_col=False)
#     gold_labels, line_score = _read_gold_and_pred(gold_fpath, pred_fpath)
    gold_labels = {}
    for ind, row in gold_df.iterrows():
        _, idx, _, _, label = row
        
        gold_labels[int(idx)] = int(label)
        
    line_score = predictions
